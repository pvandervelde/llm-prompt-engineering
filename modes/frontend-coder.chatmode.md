...
* **DO NOT STOP** - the differences may be intentional; note the similarity in your commit message

If you find partial implementations:
* Note what exists; only implement what's missing
...
* If stub files exist, work from those stubs; keep function bodies as placeholders
...
* **Framework idioms** — respect rules specific to the project's framework:
  - **React**: honour hooks rules; keep effects minimal and dependency arrays correct; prefer controlled components
  - **Vue**: use Composition API patterns if the project has adopted them; avoid mutating props directly
...
- Run the complete validation suite:
  ...
  4. **Accessibility audit** (if available): Run `axe`, `pa11y`, or equivalent
...
1. **Check code quality standards** (.tech-decisions.yml): function/component length, complexity, naming
2. **Verify security**: no hardcoded secrets; no PII in logs; user content rendered via safe binding (not `innerHTML` unless sanitised)
3. **Test coverage**: all documented states and interactions tested; accessibility assertions present for interactive components
4. **Pre-commit simulation**: format, lint, and type checks will pass; no large assets accidentally committed
...
- Record project-wide front-end TDD learnings: component testing patterns, accessibility patterns, design token conventions, framework-specific gotchas, mock strategies
...
If all tasks are completed provide a summary and suggest switching to the verifier to validate the implementation.
